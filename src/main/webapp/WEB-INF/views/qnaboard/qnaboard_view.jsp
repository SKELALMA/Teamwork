...
<body>
	...
	<form name="myform">
		<input type="hidden" name="qna_id" 	value="<%=dto.getQna_id()%>" >
        ...
        <table class="table table-hover " style="margin-top:30px;">
            <tbody>
              <tr class="table-secondary">
                <th>제목</th>
                <td colspan="6"><%=dto.getQna_title() %></td>
              </tr>
              <tr>
                <th >작성자</th>
         ...
         <!-- 
          <div class="container mt-3" style="text-align:right;">
            ...
            <a href="< %=request.getContextPath()%>/board/list" class="btn btn-secondary">수정</a>
            <a href="< %=request.getContextPath()%>/board/list" class="btn btn-secondary">삭제</a>
          </div>
          -->
          ...
          <input type="hidden" name="comment_board_id" id="board_id" value="<%=dto.getQna_id()%>" />
...
function goModify(){
	...
	var frm = document.myform;
	frm.action="<%=request.getContextPath()%>/qnaboard/modify";
	frm.submit();
}
...
function goDelete(){
	...
	if( confirm("삭제하시겠습니까?") ){
		...
		var frm = document.myform;
		frm.action="<%=request.getContextPath()%>/qnaboard/delete";
		frm.submit();
	}
}
